...
def factorize(P, S, num_factors, lambda_reg=1e-5, num_iterations=20, init_std=0.01, verbose=False, dtype='float32'):
    """
    ...
    verbose: print a bunch of stuff during training.
    ...
    for i in xrange(num_iterations):
        ...
        U = _recompute_factors(V, Sl, Dl, Il, lambda_reg, dtype=dtype)

        if verbose:
            print "    time since start: %.3f seconds" % (time.time() - start_time)
            print "    recompute item factors V"

        V = _recompute_factors(U, STl, DTl, ITl, lambda_reg, dtype=dtype)
